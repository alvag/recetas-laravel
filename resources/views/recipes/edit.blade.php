...
@section('content')
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-md-8">
            <form action="{{route('recipes.update', ['recipe' => $recipe->id])}}" enctype="multipart/form-data"
                  method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" id="category"
                        ...
                        @foreach($categories as $id => $category)
                            <option
                                value="{{$id}}"
                                {{old('category_id') == $id ? 'selected' : ($recipe->category_id == $id ? 'selected' : '')}}>
                                {{$category}}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" id="preparation"
                           name="preparation"
                           value="{{old('preparation') ? old('preparation') : $recipe->preparation}}">
                    <trix-editor
...
@section('scripts')
    <script src="https://cdnjs.cloudflare.com/ajax/libs/trix/1.2.3/trix.min.js"
            integrity="sha256-UILSuo1S5AFy4RyVZ9Xm7v8EVWLdv3IujsyN83s3wRw=" crossorigin="anonymous" defer></script>
@endsection
